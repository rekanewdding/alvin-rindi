...
// ===== COUNTDOWN TIMER =====
const weddingDate = new Date('2026-02-14T08:00:00+07:00').getTime();
...
if (shareBtn) {
    shareBtn.addEventListener('click', function() {
        const shareData = {
            title: 'Undangan Pernikahan Ike & Boby',
            text: 'Saya ingin mengundang Anda ke acara pernikahan Ike & Boby. Buka undangan digitalnya!',
        ...
        item.addEventListener('click', function() {
            ...
            lightbox.innerHTML = `
                <div style="position: relative; max-width: 90%; max-height: 90%;">
                    <img src="${imgSrc}" alt="${altText}" style="max-width: 100%; max-height: 80vh; border-radius: 10px; animation: zoomInOut 0.5s ease;">
...
function initLocationMaps() {
    ...
    openMapBtns.forEach(btn => {
        btn.addEventListener('click', function(e) {
            e.preventDefault();
            const isAkad = this.textContent.includes('Akad') || this.closest('.location-item')?.querySelector('h5')?.textContent.includes('Akad');
...
function addFloatingAnimations() {
    ...
    // Add heartbeat to all heart icons
    const hearts = document.querySelectorAll('.fa-heart:not(.beating-heart)');
    hearts.forEach(heart => {
        heart.classList.add('animate__animated', 'animate__heartBeat', 'animate__infinite');
    });
}

// ===== BUTTON ANIMATIONS (NEW) =====
function initButtonAnimations() {
    ...
    // All buttons animation on click
...
// ===== CONSOLE GREETING =====
console.log('%c🎉 Undangan Pernikahan Ike & Boby 🎉', 'font-size: 18px; color: #7A9779; font-weight: bold;');
console.log('%cDibuat dengan ❤️ oleh Berjodoh.id', 'font-size: 14px; color: #B7881E;');